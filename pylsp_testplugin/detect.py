...
class ASTWalker(ast.NodeVisitor):
    ...
    def _check_function_call(self, node):
        ...
        # return line # etc and 
        # module sql
        # node.func.att executre or execute many
        # stop here
        query = self._check_function_argument(first_argument)
        if query and re.search(SQL_OPERATORS, query):
            self.candidates.append([node.func.attr,node.lineno, node.col_offset, node.end_col_offset])

    # // dont even check function argument
